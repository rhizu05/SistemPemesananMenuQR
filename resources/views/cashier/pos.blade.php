...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('processOrder').addEventListener('click', function() {
        ...
        // Send to server (Cashier Route)
        fetch('{{ route("cashier.pos.create-order") }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                if (data.payment_method === 'qris' && data.qris_url) {
                    ...
                    // Check order receipt url - using admin receipt route if in admin context or generic if possible?
                    // But we are in cashier view now. Let's assume generic receipt logic or stick to current context.
                    // The backend typically returns order_id. We can use admin.order.receipt OR cashier route if exists.
                    // For now, let's try to detect context or fallback.
                    const receiptBaseUrl = "{{ Auth::user()->isAdmin() ? route('admin.order.receipt', ['id' => ':id']) : '/cashier/order/:id/receipt' }}"; 
                    // Actually, simpler to just use route helper
                     
                    const receiptUrl = "{{ route('cashier.order.receipt', ['id' => 'ORDER_ID']) }}".replace('ORDER_ID', data.order_id);
                    document.getElementById('printReceiptQris').href = receiptUrl;
                    ...
                    const receiptUrl = "{{ route('cashier.order.receipt', ['id' => 'ORDER_ID']) }}".replace('ORDER_ID', data.order_id);
                    document.getElementById('printReceipt').href = receiptUrl;
...
function checkPaymentStatus(isManual = false) {
    ...
    // Using admin check status route because it exists, and admin has access.
    // If we want pure cashier route, we need to create it.
    // Let's use the route we created earlier 'admin.order.check-status' but mapped to logic.
    // Wait, user wants 'cashier'.
    // Let's assume we will add cashier.order.check-status
    fetch(`/cashier/order/${window.currentOrderId}/status`)
        ...
        .then(data => {
            if (data.status === 'paid') {
                ...
                const receiptUrl = "{{ route('cashier.order.receipt', ['id' => 'ORDER_ID']) }}".replace('ORDER_ID', window.currentOrderId);
                document.getElementById('printReceipt').href = receiptUrl;
    ...
    <div class="mt-3 ms-3">
        ...
        @else
        <a href="{{ route('cashier.dashboard') }}" class="btn text-white" 
           style="background-color: #4A7F5A; transition: all 0.3s;"
           onmouseover="this.style.backgroundColor='#3d6b4a'; this.style.transform='translateY(-2px)';" 
           onmouseout="this.style.backgroundColor='#4A7F5A'; this.style.transform='translateY(0)';">
            <i class="bi bi-arrow-left"></i> Kembali ke Dashboard
